...
export function Shop() {
  ...
  return <main className="page-width py-12">
    <p className="eyebrow">OUR SOFTEST COLLECTION</p><h1 className="font-display text-4xl font-bold md:text-6xl">Shop all</h1>
...
export function ProductDetails() {
  ...
  return <main className="page-width py-8 md:py-12">
    ...
    <div className="grid gap-9 lg:grid-cols-2">
      ...
      <div className="lg:pl-8"><p className="eyebrow">{String(product.category).replaceAll('-', ' ')}</p><h1 className="font-display text-4xl font-bold leading-tight md:text-5xl">{product.name}</h1>
        ...
        <div className="mt-7 flex gap-3"><div className="flex items-center rounded-full border border-cocoa/15 bg-white"><button className="p-3" onClick={() => setQuantity(Math.max(1, quantity-1))}><Minus size={16} /></button><span className="w-8 text-center">{quantity}</span><button className="p-3" onClick={() => setQuantity(quantity+1)}><Plus size={16} /></button></div><button onClick={() => addToCart(product, selectedVariant?._id, quantity)} className="button-primary flex-1"><ShoppingBag size={18} /> Add to cart</button><button onClick={() => toggleWishlist(id)} className="icon-button h-12 w-12 border border-cocoa/15 bg-white"><Heart fill={wishlist.includes(id) ? 'currentColor' : 'none'} /></button></div>
      ...
      <div className="space-y-6">
        <fieldset className="form-card">
          ...
          {addressesLoading ? <p className="text-sm text-cocoa/50">Loading saved addresses…</p> : <>
            ...
            <div className="mt-4 flex flex-wrap gap-3"><button type="button" className="button-secondary" onClick={() => { setAddressError(''); setAddressFormOpen(true) }}><Plus size={17}/> Add new address</button>{!!addresses.length&&<Link to="/account/addresses" className="button-secondary">Manage addresses</Link>}</div>
          </>}
        </fieldset>
        <fieldset className="form-card">
